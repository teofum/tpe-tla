...
bloque más cercano con reglas similares a otros lenguajes imperativos (e.g. C).
Cada variable tiene un tipo y un valor; las variables deberán ser inicializadas
explícitamente al definirlas y no hacerlo será un error.
...
\begin{verbatim}
  ...
  x :: 5               // Declara una constante "x", se infiere tipo "int"
  x = 6                // ERROR: no se puede asignar una constante
\end{verbatim}
\normalsize
...
Una función admite composición únicamente si contiene la palabra
\texttt{compose}; de no tenerla pasar una composición es un error.
